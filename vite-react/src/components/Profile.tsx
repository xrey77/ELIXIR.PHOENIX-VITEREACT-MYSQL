...
export default function Profile() {    
    ...
    const fetchUserData = (id: any, token: any) => {
        ...
        .then((res: any) => {
            ...
            let pix: any = `http://127.0.0.1:4000/users/${res.data.userpic}`;
            setUserpicture(pix);

            if (res.data.qrcodeurl !== null) {
                let qrcode: any = `http://127.0.0.1:4000/qrcodes/${res.data.qrcodeurl}`;
                setQrcodeurl(qrcode);
            } else {
                setQrcodeurl('http://127.0.0.1:4000/images/qrcode.png');
            }
    ...
    const changePicture = (event: any) => {
        event.preventDefault();
            ...
            .then((res: any) => {
                setProfileMsg(res.data.message);
                window.setTimeout(() => {
                    let pic: any = `http://127.0.0.1:4000/users/${res.data.userpic}`;
    ...
    const disableMFA = () => {
        ...
        .then((res: any) => {
            setProfileMsg(res.data.message);
            let pic: any = 'http://127.0.0.1:4000/images/qrcode.png';
            setQrcodeurl(pic);                
            setTimeout(() => {
                setProfileMsg('');
            },3000);
